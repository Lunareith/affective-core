...
class EmotionState:
    ...
    def update_vec(self, new_vec: dict[str, float]) -> None:
        ...
        for dim in self.dimensions:
            self.vec[dim] = float(new_vec.get(dim, self.baseline.get(dim, 0.0)))
        self.last_update = datetime.now(timezone.utc).isoformat()
        self.save()
    ...
    def _rotate_backups(self) -> None:
        ...
        bak1 = self.backup_dir / "current.bak1.json"
        bak2 = self.backup_dir / "current.bak2.json"
        bak3 = self.backup_dir / "current.bak3.json"
    ...
    def _restore_from_backup(self) -> dict | None:
        ...
        for bak_name in ("current.bak1.json", "current.bak2.json", "current.bak3.json"):
